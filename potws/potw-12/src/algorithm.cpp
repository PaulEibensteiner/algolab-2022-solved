...
typedef boost::rational<long> rating;
typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::directedS,
  boost::no_property,                         // no vertex property
  boost::property<boost::edge_weight_t, unsigned int,
	boost::property<boost::edge_weight2_t, rating>>  // interior edge weight property
> weighted_graph;
...
rating get_weight_sum(edge_desc edge) {
	if (weight_sums[edge] != -1) {
		return weight_sums[edge];
	}

	long numerator = weights[edge];
	long denominator = 1;
  ...
  for (boost::tie(oe_beg, oe_end) = boost::out_edges(target, g); oe_beg != oe_end; ++oe_beg) {
		rating edge_weight_sum = get_weight_sum(*oe_beg);
    denominator += edge_weight_sum.denominator();
		numerator += edge_weight_sum.numerator();
  }
	rating weight_sum = rating(numerator, denominator);
...
void testcase() {
  ...
  weights = boost::get(boost::edge_weight, g);
	...
	for (int i = 0; i < n; i++)
	{
		...
		weights[e] = d;
		weight_sums[e] = boost::rational<long>(-1, 1);
	}
